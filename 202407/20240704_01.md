...
备注: 还可以解决根据来源IP拒绝访问的问题, 例如openai.com, 使用macOS的网络配置-代理配置-http/https代理, 配置好后使用自带safari浏览器即可访问openai.com. 使用chrome zeroomega插件灵活控制是否使用socks5代理, safari浏览器就用来访问那种追踪来源IP的网站.  
...
# http_access allow localhost 上面加上这条
...
if pgrep -x "ssh" >/dev/null && [ $(ps -efw|grep -v grep|grep ssh|grep CqtgfnN|grep -c $localmapport) -eq 1 ] 
then
echo "ssh 进程正在运行"
else
func_ssh
...
如果要在容器中使用宿主机的ssh隧道代理通过远程squid http代理访问外网, 注意, 如果你只开启了 http_access allow localhost, 建议还加上这条. http_access allow localnet
