...
根据 Paimon 官方文档，Paimon 主键表采用 **LSM 树** 结构，支持三种表模式：  
- **MOR (Merge On Read)** ：默认模式，写入延迟低但读取需多路合并  
- **COW (Copy On Write)** ：写入时完整合并，读取性能最佳但写入放大严重  
- **MOW (Merge On Write)** ：写入时生成删除向量，读写性能平衡  
  
Retina 的核心价值： **10毫秒级数据新鲜度 + 320万行/秒的变更重放吞吐量**  
  
| 对比维度 | **Pixels-Retina** | **Paimon MOW** | **Paimon MOR** | **Paimon COW** |  
|---------|-------------------|----------------|----------------|----------------|  
| **数据可见延迟** | **<10ms** | 5-30秒 | 0秒（注1） | 0秒 |  
| **写入性能** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐ |  
| **读取性能** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |  
| **写入放大** | 极低 | 中等 | 低 | 严重 |  
| **核心机制** | 可见性位图 | 删除向量 | 小规模合并 | 全量合并 |  
  
> 注1：MOR 模式数据写入 L0 后立即可见，但读取时需要多路合并，查询延迟较高。  
...
根据 Paimon 官方文档（1.2版本），Paimon 主键表采用 LSM 树 结构，支持三种模式。  
Pixels-Retina 与 Paimon 三种模式的详细对比如下：  
  
| 特性 | **Pixels-Retina** | **Paimon MOW** | **Paimon MOR** | **Paimon COW** |  
|------|-------------------|----------------|----------------|----------------|  
| **数据可见延迟** | **<10ms** | 5-30秒（注1） | 0秒（注2） | 0秒 |  
| **写入性能** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐ |  
| **读取性能** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |  
| **写入放大** | 极低 | 中等 | 低 | 严重 |  
| **变更粒度** | 行级 | 行级 | 行级 | 行级 |  
| **MVCC 机制** | 轻量级删除链 | 删除向量 | 小规模合并 | 全量合并 |  
| **LSM 结构** | 无 | 有 | 有 | 有 |  
| **索引支持** | 多版本主索引 | 主键索引 | 主键索引 | 主键索引 |  
| **存储格式** | 自研 Pixels 列存 | Parquet/ORC | Parquet/ORC | Parquet/ORC |  
| **CDC 支持** | 原生 CDC 重放 | 原生支持 | 原生支持 | 原生支持 |  
  
> 注1：MOW 模式若启用异步 compaction，数据可见延迟可达 1-10 分钟。  
> 注2：MOR 模式数据写入 L0 后立即可见，但读取时需多路合并。  
...
│                                                                             │  
│  写入流程:                                                                  │  
│  数据写入 → Flink Checkpoint → L0 文件 → 触发 Compaction → 合并到更高层级    │  
│                                                                             │  
│  L0: 最新刷盘的小文件层，最未合并                                            │  
│  L1...Lmax: 越高层文件越少、越大、越稳定                                    │  
└─────────────────────────────────────────────────────────────────────────────┘  
```  
  
#### MOR (Merge On Read) - 默认模式  
  
| 维度 | 表现 | 说明 |  
|------|------|------|  
| **写入性能** | ⭐⭐⭐⭐⭐ 优秀 | 只做小规模合并，写入延迟低 |  
| **读取性能** | ⭐⭐ 一般 | 需要多路合并，读取延迟较高 |  
| **数据可见延迟** | 0秒（注3） | 数据写入 L0 后立即可见 |  
| **适用场景** | 高写入吞吐、日志收集、IoT 数据 |  
  
> 注3：MOR 数据立即可见，但读取时需要扫描所有层并合并，查询性能差。  
  
#### COW (Copy On Write)  
  
| 维度 | 表现 | 说明 |  
|------|------|------|  
| **写入性能** | ⭐ 较差 | 每次写入都完整合并，写入放大严重 |  
| **读取性能** | ⭐⭐⭐⭐⭐ 优秀 | 无需合并，直接读取，延迟最低 |  
| **数据可见延迟** | 0秒 | 写入时已完整合并 |  
| **适用场景** | 读多写少、配置表、维度表、小数据量 |  
  
#### MOW (Merge On Write) - 删除向量模式  
  
| 维度 | 表现 | 说明 |  
|------|------|------|  
| **写入性能** | ⭐⭐⭐⭐ 良好 | 需查询 LSM 生成删除向量，无完整合并 |  
| **读取性能** | ⭐⭐⭐⭐ 良好 | 删除向量过滤，减少多路合并开销 |  
| **数据可见延迟** | 5-30秒 | 同步 compaction；异步可达 1-10 分钟 |  
| **适用场景** | 频繁更新、平衡读写、订单状态表 |  
...
┌─────────────────────────────────────────────────────────────────┐  
│ Write Path:                                                     │  
│   写入数据 → 查询 LSM 树 → 生成删除向量文件 → 标记旧数据为删除  │  
│                                                                 │  
│ Read Path:                                                      │  
│   读取数据文件 → 应用删除向量过滤 → 返回结果                     │  
│   优势: 读取性能好，无需多路合并                                 │  
│   代价: 数据可见延迟 5-30 秒                                     │  
└─────────────────────────────────────────────────────────────────┘  
...
| 指标 | Pixels-Retina | Paimon MOW | Paimon MOR | Paimon COW |  
|------|---------------|------------|------------|------------|  
| **数据新鲜度** | **<10ms** | 5-30秒 | 0秒 | 0秒 |  
| **变更重放吞吐** | 3.2M rows/s | 较高 | 最高 | 较低 |  
| **查询延迟** | 低 | 中等 | 较高 | 低 |  
| **写入放大** | 极低 | 中等 | 低 | 严重 |  
| **存储成本** | 较低 | 中等 | 低 | 中等 |  
...
┌─────────────────────────────────────────────────────────────────┐  
│ MySQL (源)  ──CDC──▶  Kafka  ──▶  Flink  ──▶  Paimon (MOW)    │  
│                                            │                    │  
│                                            ▼                    │  
│                                   写入数据                       │  
│                                            │                    │  
...
注: 若使用 Paimon MOR 模式，订单状态可立即可见，但查询时需多路合并  
```  
...
│  ─────────────────────────────────────────────────────────────          │  
│                                                                         │  
│  ✅ 极致实时 (<100ms)              → Pixels-Retina                     │  
│     例: 实时大屏、金融交易监控、库存更新                                   │  
│                                                                         │  
│  🟡 准实时 (秒级)                  → Paimon MOW                        │  
│     例: 订单状态、用户画像、实时报表                                       │  
│                                                                         │  
│  🟢 可接受分钟级延迟                 → Paimon MOR/COW                    │  
│     例: 日志分析、审计报表、历史数据                                       │  
│                                                                         │  
│  🔵 读多写少                        → Paimon COW                        │  
│     例: 配置表、商品信息表、小数据量维度表                                 │  
...
**混用策略**：实际生产环境中，可以根据表类型混用：  
  
| 表类型 | 推荐方案 | 理由 |  
|--------|----------|------|  
| 实时状态表 | Pixels-Retina | 需要 <100ms 级别的实时性 |  
| 业务主表 | Paimon MOW | 秒级延迟可接受，平衡读写 |  
| 日志流水表 | Paimon MOR | 高写入吞吐，查询较少 |  
| 维表/配置表 | Paimon COW | 读多写少，查询性能优先 |  
